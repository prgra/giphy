...
// TrendingPg returns a trending response from the Giphy API
func (c *Client) TrendingPg(page int, lim int) (Trending, error) {
	path := fmt.Sprintf("/gifs/trending?limit=%d&offset=%d", lim, page*lim)
	req, err := c.NewRequest(path)
	if err != nil {
		return Trending{}, err
	}
